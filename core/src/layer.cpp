...
const Tensor &RegularizationLayer::backward(const Tensor &input,
  ...
  for (int i = 0; i < grad_output.rows * grad_output.cols; ++i) {
    scalar penalty = 0.0f;
    if (type == 1) {
      penalty = lambda * ((input.data[i] > 0.0f)
                              ? 1.0f
                              : ((input.data[i] < 0.0f) ? -1.0f : 0.0f));
    } else if (type == 2) {
...
RNNLayer::RNNLayer(int input_dim, int hidden_dim, int seq_len,
                   bool return_sequences)
    : input_dim(input_dim), hidden_dim(hidden_dim), seq_len(seq_len),
      return_sequences(return_sequences), weights_ih(input_dim, hidden_dim),
      weights_hh(hidden_dim, hidden_dim), biases(1, hidden_dim),
      grad_weights_ih(input_dim, hidden_dim),
      grad_weights_hh(hidden_dim, hidden_dim), grad_biases(1, hidden_dim) {
...
const Tensor &RNNLayer::forward(const Tensor &input) {
  ...
  // Single parallel_for dispatch over the batch, running all sequence steps
  // in-place
...
const Tensor &RNNLayer::backward(const Tensor &input,
                                 const Tensor &grad_output) {
  int batch_size = input.rows / seq_len;
  ensure_grad_input_dims(batch_size * seq_len, input_dim);

  std::memset(grad_weights_ih.data, 0,
              grad_weights_ih.rows * grad_weights_ih.cols * sizeof(scalar));
  std::memset(grad_weights_hh.data, 0,
              grad_weights_hh.rows * grad_weights_hh.cols * sizeof(scalar));
  std::memset(grad_biases.data, 0,
              grad_biases.rows * grad_biases.cols * sizeof(scalar));
  std::memset(grad_input.data, 0,
              grad_input.rows * grad_input.cols * sizeof(scalar));
...
#pragma omp parallel for schedule(static)
  for (int b = 0; b < batch_size; ++b) {
    ...
    for (int s = seq_len - 1; s >= 0; --s) {
      ...
      for (int h = 0; h < hidden_dim; ++h) {
        scalar dh = dh_next[h];
        if (return_sequences)
          dh += grad_output.data[row_idx * hidden_dim + h];
        else if (s == seq_len - 1)
          dh += grad_output.data[b * hidden_dim + h];
      ...
      for (int prev_h = 0; prev_h < hidden_dim; ++prev_h) {
        scalar dnext = 0.0f;
        for (int h = 0; h < hidden_dim; ++h) {
          dnext += d_tanh_all.data[row_idx * hidden_dim + h] *
                   weights_hh.data[prev_h * hidden_dim + h];
        }
  ...
  ThreadPool::getInstance().parallel_for(
      0, input_dim, [this, &input, &d_tanh_all, batch_size](int i) {
  ...
  ThreadPool::getInstance().parallel_for(
      0, hidden_dim, [this, &d_tanh_all, batch_size](int prev_h_idx) {
        scalar *grad_row = &grad_weights_hh.data[prev_h_idx * hidden_dim];
        for (int b = 0; b < batch_size; ++b) {
          int base = b * seq_len;
          for (int s = 1; s < seq_len; ++s) {
            scalar h_prev_val =
                h_states.data[(base + s - 1) * hidden_dim + prev_h_idx];
            if (h_prev_val == 0)
              continue;
            const scalar *d_tanh_row =
                &d_tanh_all.data[(base + s) * hidden_dim];
            for (int h = 0; h < hidden_dim; ++h) {
  ...
  ThreadPool::getInstance().parallel_for(
      0, batch_size * seq_len, [this, &d_tanh_all](int r) {
        for (int i = 0; i < input_dim; ++i) {
          scalar sum = 0;
          for (int h = 0; h < hidden_dim; ++h) {
            sum += d_tanh_all.data[r * hidden_dim + h] *
                   weights_ih.data[i * hidden_dim + h];
          }
...
LSTMLayer::LSTMLayer(int input_dim, int hidden_dim, int seq_len,
                     bool return_sequences)
    : input_dim(input_dim), hidden_dim(hidden_dim), seq_len(seq_len),
      return_sequences(return_sequences), weights_ih(input_dim, 4 * hidden_dim),
  ...
  // Forget gate bias = 1.0 (Keras unit_forget_bias default, gate order:
  // i,f,o,g)
...
const Tensor &LSTMLayer::backward(const Tensor &input,
                                  const Tensor &grad_output) {
  int batch_size = input.rows / seq_len;
  ensure_grad_input_dims(batch_size * seq_len, input_dim);
  ...
  std::memset(d_gates_all.data, 0,
              d_gates_all.rows * d_gates_all.cols * sizeof(scalar));
...
#pragma omp parallel for schedule(static)
  for (int b = 0; b < batch_size; ++b) {
    ...
    for (int s = seq_len - 1; s >= 0; --s) {
      ...
      for (int h = 0; h < hidden_dim; ++h) {
        ...
        scalar prev_c_val =
            (s > 0) ? c_states.data[(row_idx - 1) * hidden_dim + h] : 0.0f;
        ...
        d_gates_all.data[row_idx * 4 * hidden_dim + h] =
            di_gate * i * (1.0f - i);
        d_gates_all.data[row_idx * 4 * hidden_dim + hidden_dim + h] =
            df_gate * f * (1.0f - f);
        d_gates_all.data[row_idx * 4 * hidden_dim + 2 * hidden_dim + h] =
            do_gate * o * (1.0f - o);
        d_gates_all.data[row_idx * 4 * hidden_dim + 3 * hidden_dim + h] =
            dg_gate * (1.0f - g * g);
      }
      ...
      for (int prev_h = 0; prev_h < hidden_dim; ++prev_h) {
        scalar dnext = 0.0f;
        for (int h = 0; h < 4 * hidden_dim; ++h) {
          dnext += d_gates_all.data[row_idx * 4 * hidden_dim + h] *
                   weights_hh.data[prev_h * 4 * hidden_dim + h];
        }
        dh_next[prev_h] = dnext;
      }
    }
  ...
  ThreadPool::getInstance().parallel_for(
      0, input_dim, [this, &input, &d_gates_all, batch_size](int i) {
  ...
  ThreadPool::getInstance().parallel_for(
      0, hidden_dim, [this, &d_gates_all, batch_size](int prev_h_idx) {
        ...
        for (int b = 0; b < batch_size; ++b) {
          ...
          for (int s = 1; s < seq_len; ++s) {
            ...
            if (h_prev_val == 0)
              continue;
            const scalar *d_gates_row =
                &d_gates_all.data[(base + s) * 4 * hidden_dim];
            for (int g = 0; g < 4 * hidden_dim; ++g) {
  ...
  ThreadPool::getInstance().parallel_for(
      0, batch_size * seq_len, [this, &d_gates_all](int r) {
        for (int i = 0; i < input_dim; ++i) {
          scalar sum = 0;
          for (int g = 0; g < 4 * hidden_dim; ++g) {
            sum += d_gates_all.data[r * 4 * hidden_dim + g] *
                   weights_ih.data[i * 4 * hidden_dim + g];
          }
          grad_input.data[r * input_dim + i] = sum;
        }
      });
  ...
  // grad_biases
  ThreadPool::getInstance().parallel_for(
      0, 4 * hidden_dim, [this, &d_gates_all, batch_size](int g) {
        scalar sum = 0;
        for (int r = 0; r < batch_size * seq_len; ++r) {
          sum += d_gates_all.data[r * 4 * hidden_dim + g];
        }
...
LayerNormLayer::LayerNormLayer(int feature_dim, scalar epsilon)
    : feature_dim(feature_dim), epsilon(epsilon), gamma(1, feature_dim),
      beta(1, feature_dim), grad_gamma(1, feature_dim),
      grad_beta(1, feature_dim) {
...
const Tensor &LayerNormLayer::forward(const Tensor &input) {
  ...
  ThreadPool::getInstance().parallel_for(0, input.rows, [this, &input](int i) {
    ...
    for (int j = 0; j < feature_dim; ++j) {
      x_norm.data[i * feature_dim + j] =
          x_centered.data[i * feature_dim + j] / std_val;
      output.data[i * feature_dim + j] =
          gamma.data[j] * x_norm.data[i * feature_dim + j] + beta.data[j];
    }
  });
...
const Tensor &LayerNormLayer::backward(const Tensor &input,
                                       const Tensor &grad_output) {
  ensure_grad_input_dims(input.rows, input.cols);
  ...
  for (int i = 0; i < input.rows; ++i) {
    for (int j = 0; j < feature_dim; ++j) {
      grad_gamma.data[j] += grad_output.data[i * feature_dim + j] *
                            x_norm.data[i * feature_dim + j];
  ...
  ThreadPool::getInstance().parallel_for(
      0, input.rows, [this, &grad_output](int i) {
        scalar sum_dx_norm = 0;
        scalar sum_dx_norm_x_centered = 0;
        for (int j = 0; j < feature_dim; ++j) {
          scalar dx_norm =
              grad_output.data[i * feature_dim + j] * gamma.data[j];
          sum_dx_norm += dx_norm;
          sum_dx_norm_x_centered +=
              dx_norm * x_centered.data[i * feature_dim + j];
        }
        ...
        for (int j = 0; j < feature_dim; ++j) {
          scalar dx_norm =
              grad_output.data[i * feature_dim + j] * gamma.data[j];
          scalar dx = (m * dx_norm - sum_dx_norm -
                       x_centered.data[i * feature_dim + j] *
                           sum_dx_norm_x_centered / var_eps) /
                      (m * std_val);
...
void LayerNormLayer::save(std::ostream &os) const {
  gamma.save(os);
  beta.save(os);
}
void LayerNormLayer::load(std::istream &is) {
  gamma.load(is);
  beta.load(is);
}
...
SelfAttentionLayer::SelfAttentionLayer(int embed_dim, int seq_len)
    : embed_dim(embed_dim), seq_len(seq_len), W_q(embed_dim, embed_dim),
      W_k(embed_dim, embed_dim), W_v(embed_dim, embed_dim), b_q(1, embed_dim),
      b_k(1, embed_dim), b_v(1, embed_dim), grad_W_q(embed_dim, embed_dim),
      grad_W_k(embed_dim, embed_dim), grad_W_v(embed_dim, embed_dim),
  ...
  ThreadPool::getInstance().parallel_for(0, batch_size, [this, scale](int b) {
    for (int i = 0; i < seq_len; ++i) {
      ...
      for (int j = 0; j < seq_len; ++j) {
        ...
        for (int d = 0; d < embed_dim; ++d) {
          dot += Q.data[(b * seq_len + i) * embed_dim + d] *
                 K.data[(b * seq_len + j) * embed_dim + d];
        }
        scalar s = dot * scale;
      ...
      for (int j = 0; j < seq_len; ++j) {
        scalar e =
            std::exp(scores.data[(b * seq_len + i) * seq_len + j] - max_score);
    ...
    for (int i = 0; i < seq_len; ++i) {
      for (int d = 0; d < embed_dim; ++d) {
        scalar sum = 0.0f;
        for (int j = 0; j < seq_len; ++j) {
          sum += attention_weights.data[(b * seq_len + i) * seq_len + j] *
                 V.data[(b * seq_len + j) * embed_dim + d];
        }
...
const Tensor &SelfAttentionLayer::backward(const Tensor &input,
                                           const Tensor &grad_output) {
  int batch_size = input.rows / seq_len;
  ensure_grad_input_dims(input.rows, input.cols);
  ...
  ThreadPool::getInstance().parallel_for(
      0, batch_size, [this, &grad_output, &dQ, &dK, &dV, scale](int b) {
        for (int i = 0; i < seq_len; ++i) {
          for (int j = 0; j < seq_len; ++j) {
            scalar w = attention_weights.data[(b * seq_len + i) * seq_len + j];
            for (int d = 0; d < embed_dim; ++d) {
              dV.data[(b * seq_len + j) * embed_dim + d] +=
                  w * grad_output.data[(b * seq_len + i) * embed_dim + d];
            }
          }
        }

        // Precompute per-query scalar: sum_k a_ik * (dout_i · v_k) — O(n²) not
        // O(n³)
        ...
        for (int i = 0; i < seq_len; ++i) {
          for (int k = 0; k < seq_len; ++k) {
            ...
            scalar dot = 0.0f;
            for (int d = 0; d < embed_dim; ++d)
              dot += grad_output.data[(b * seq_len + i) * embed_dim + d] *
                     V.data[(b * seq_len + k) * embed_dim + d];
            scalar_i[i] += wk * dot;
          }
        }
        ...
        for (int i = 0; i < seq_len; ++i) {
          for (int j = 0; j < seq_len; ++j) {
            ...
            scalar dscore = 0.0f;
            for (int d = 0; d < embed_dim; ++d)
              dscore += grad_output.data[(b * seq_len + i) * embed_dim + d] *
                        V.data[(b * seq_len + j) * embed_dim + d];

            scalar ds = w * (dscore - scalar_i[i]) * scale;
            for (int d = 0; d < embed_dim; ++d) {
              dQ.data[(b * seq_len + i) * embed_dim + d] +=
                  ds * K.data[(b * seq_len + j) * embed_dim + d];
              dK.data[(b * seq_len + j) * embed_dim + d] +=
                  ds * Q.data[(b * seq_len + i) * embed_dim + d];
            }
          }
        }
      });
...
void SelfAttentionLayer::update(Optimizer *opt) {
  if (opt) {
    opt->update(W_q, grad_W_q);
    opt->update(b_q, grad_b_q);
    opt->update(W_k, grad_W_k);
    opt->update(b_k, grad_b_k);
    opt->update(W_v, grad_W_v);
    opt->update(b_v, grad_b_v);
  }
}
void SelfAttentionLayer::save(std::ostream &os) const {
  W_q.save(os);
  W_k.save(os);
  W_v.save(os);
  b_q.save(os);
...
void SelfAttentionLayer::load(std::istream &is) {
  W_q.load(is);
  W_k.load(is);
  W_v.load(is);
  b_q.load(is);
...
TransformerEncoderLayer::TransformerEncoderLayer(int embed_dim, int seq_len,
                                                 int ff_dim)
    : embed_dim(embed_dim), seq_len(seq_len),
      ff_dim(ff_dim == -1 ? 4 * embed_dim : ff_dim),
...
const Tensor &TransformerEncoderLayer::backward(const Tensor &input,
                                                const Tensor &grad_output) {
  ensure_grad_input_dims(input.rows, input.cols);
...
void TransformerEncoderLayer::load(std::istream &is) {
  attention.load(is);
  norm1.load(is);
  ff1.load(is);
  ff2.load(is);
...
const Tensor &MaskedSelfAttentionLayer::backward(const Tensor &input,
                                                 const Tensor &grad_output) {
  int batch_size = input.rows / seq_len;
  ensure_grad_input_dims(input.rows, input.cols);
...
void MaskedSelfAttentionLayer::save(std::ostream &os) const {
  W_q.save(os);
  W_k.save(os);
  W_v.save(os);
  b_q.save(os);
...
void MaskedSelfAttentionLayer::load(std::istream &is) {
  W_q.load(is);
  W_k.load(is);
  W_v.load(is);
  b_q.load(is);
...
TransformerDecoderLayer::TransformerDecoderLayer(int embed_dim, int seq_len,
                                                 int ff_dim)
    : embed_dim(embed_dim), seq_len(seq_len),
      ff_dim(ff_dim == -1 ? 4 * embed_dim : ff_dim),
      attention(embed_dim, seq_len), norm1(embed_dim),
...
const Tensor &TransformerDecoderLayer::backward(const Tensor &input,
                                                const Tensor &grad_output) {
  ensure_grad_input_dims(input.rows, input.cols);
...
void TransformerDecoderLayer::save(std::ostream &os) const {
  attention.save(os);
  norm1.save(os);
  ff1.save(os);
  ff2.save(os);
...
void TransformerDecoderLayer::load(std::istream &is) {
  attention.load(is);
  norm1.load(is);
  ff1.load(is);
  ff2.load(is);
...
MaskedMultiHeadAttentionLayer::MaskedMultiHeadAttentionLayer(int embed_dim,
                                                             int seq_len,
                                                             int num_heads)
    : embed_dim(embed_dim), seq_len(seq_len), num_heads(num_heads),
      head_dim(embed_dim / num_heads), W_q(embed_dim, embed_dim),
      W_k(embed_dim, embed_dim), W_v(embed_dim, embed_dim), b_q(1, embed_dim),
      b_k(1, embed_dim), b_v(1, embed_dim), W_o(embed_dim, embed_dim),
      b_o(1, embed_dim), grad_W_q(embed_dim, embed_dim),
      grad_W_k(embed_dim, embed_dim), grad_W_v(embed_dim, embed_dim),
      grad_b_q(1, embed_dim), grad_b_k(1, embed_dim), grad_b_v(1, embed_dim),
...
const Tensor &MaskedMultiHeadAttentionLayer::forward(const Tensor &input) {
  ...
  if (scores.rows != batch_size * num_heads * seq_len ||
      scores.cols != seq_len) {
  ...
  ThreadPool::getInstance().parallel_for(
      0, batch_size * num_heads, [this, scale](int task_idx) {
        ...
        for (int i = 0; i < seq_len; ++i) {
          ...
          for (int j = 0; j < seq_len; ++j) {
            if (j > i) {
              scores.data[(task_idx * seq_len + i) * seq_len + j] =
                  -1e9f; // causal mask
              continue;
            }
            scalar dot = 0.0f;
            for (int d = 0; d < head_dim; ++d) {
              dot += Q.data[(b * seq_len + i) * embed_dim + head_offset + d] *
                     K.data[(b * seq_len + j) * embed_dim + head_offset + d];
            }
            scalar s = dot * scale;
          ...
          for (int j = 0; j <= i; ++j) {
            scalar e =
                std::exp(scores.data[(task_idx * seq_len + i) * seq_len + j] -
                         max_score);
            attention_weights.data[(task_idx * seq_len + i) * seq_len + j] = e;
            sum_exp += e;
          }
          for (int j = i + 1; j < seq_len; ++j) {
            attention_weights.data[(task_idx * seq_len + i) * seq_len + j] =
                0.0f;
          }
          for (int j = 0; j <= i; ++j) {
            attention_weights.data[(task_idx * seq_len + i) * seq_len + j] /=
          ...
          for (int d = 0; d < head_dim; ++d) {
            ...
            for (int j = 0; j <= i; ++j) {
              sum += attention_weights
                         .data[(task_idx * seq_len + i) * seq_len + j] *
                     V.data[(b * seq_len + j) * embed_dim + head_offset + d];
            }
            concat_out.data[(b * seq_len + i) * embed_dim + head_offset + d] =
                sum;
          }
        }
      });
...
const Tensor &
MaskedMultiHeadAttentionLayer::backward(const Tensor &input,
                                        const Tensor &grad_output) {
  int batch_size = input.rows / seq_len;
  ensure_grad_input_dims(input.rows, input.cols);
  ...
  for (int j = 0; j < embed_dim; ++j) {
    scalar sum = 0.0f;
    for (int i = 0; i < input.rows; ++i) {
      sum += grad_output.data[i * embed_dim + j];
    }
    grad_b_o.data[j] = sum;
  }
  ...
  for (int d1 = 0; d1 < embed_dim; ++d1) {
    for (int d2 = 0; d2 < embed_dim; ++d2) {
      scalar sum = 0;
      for (int i = 0; i < input.rows; ++i) {
        sum += concat_out.data[i * embed_dim + d1] *
               grad_output.data[i * embed_dim + d2];
      }
  ...
  for (int i = 0; i < input.rows; ++i) {
    for (int d1 = 0; d1 < embed_dim; ++d1) {
      scalar sum = 0;
      for (int d2 = 0; d2 < embed_dim; ++d2) {
        sum += grad_output.data[i * embed_dim + d2] *
               W_o.data[d1 * embed_dim + d2];
      }
  ...
  ThreadPool::getInstance().parallel_for(
      0, batch_size * num_heads,
      [this, &d_concat, &dQ, &dK, &dV, scale](int task_idx) {
        int b = task_idx / num_heads;
        int h = task_idx % num_heads;
        int head_offset = h * head_dim;
        ...
        for (int i = 0; i < seq_len; ++i) {
          for (int j = 0; j <= i; ++j) {
            scalar w =
                attention_weights.data[(task_idx * seq_len + i) * seq_len + j];
            for (int d = 0; d < head_dim; ++d) {
              dV.data[(b * seq_len + j) * embed_dim + head_offset + d] +=
                  w *
                  d_concat
                      .data[(b * seq_len + i) * embed_dim + head_offset + d];
            }
          }
        }
        ...
        for (int i = 0; i < seq_len; ++i) {
          for (int k = 0; k <= i; ++k) {
            scalar wk =
                attention_weights.data[(task_idx * seq_len + i) * seq_len + k];
            scalar dot = 0.0f;
            for (int d = 0; d < head_dim; ++d)
              dot +=
                  d_concat
                      .data[(b * seq_len + i) * embed_dim + head_offset + d] *
                  V.data[(b * seq_len + k) * embed_dim + head_offset + d];
            scalar_i[i] += wk * dot;
          }
        }
        ...
        for (int i = 0; i < seq_len; ++i) {
          for (int j = 0; j <= i; ++j) {
            ...
            scalar ds = w * (dscore - scalar_i[i]) * scale;
            for (int d = 0; d < head_dim; ++d) {
              dQ.data[(b * seq_len + i) * embed_dim + head_offset + d] +=
                  ds * K.data[(b * seq_len + j) * embed_dim + head_offset + d];
              dK.data[(b * seq_len + j) * embed_dim + head_offset + d] +=
                  ds * Q.data[(b * seq_len + i) * embed_dim + head_offset + d];
            }
          }
        }
      });
  ...
  for (int d = 0; d < embed_dim; ++d) {
    scalar sq = 0, sk = 0, sv = 0;
    for (int i = 0; i < input.rows; ++i) {
      sq += dQ.data[i * embed_dim + d];
      sk += dK.data[i * embed_dim + d];
      sv += dV.data[i * embed_dim + d];
    }
    grad_b_q.data[d] = sq;
    grad_b_k.data[d] = sk;
    grad_b_v.data[d] = sv;
...
void MaskedMultiHeadAttentionLayer::update(Optimizer *opt) {
  if (opt) {
    ...
    opt->update(W_v, grad_W_v);
    opt->update(b_v, grad_b_v);
    opt->update(W_o, grad_W_o);
    opt->update(b_o, grad_b_o);
  }
}
void MaskedMultiHeadAttentionLayer::save(std::ostream &os) const {
  W_q.save(os);
  W_k.save(os);
  W_v.save(os);
  W_o.save(os);
...
void MaskedMultiHeadAttentionLayer::load(std::istream &is) {
  W_q.load(is);
  W_k.load(is);
  W_v.load(is);
  W_o.load(is);
...
TransformerMHDecoderLayer::TransformerMHDecoderLayer(int embed_dim, int seq_len,
                                                     int ff_dim, int num_heads)
    : embed_dim(embed_dim), seq_len(seq_len),
      ff_dim(ff_dim == -1 ? 4 * embed_dim : ff_dim), num_heads(num_heads),
...
const Tensor &TransformerMHDecoderLayer::backward(const Tensor &input,
                                                  const Tensor &grad_output) {
  ensure_grad_input_dims(input.rows, input.cols);
...
void TransformerMHDecoderLayer::save(std::ostream &os) const {
  attention.save(os);
  norm1.save(os);
  ff1.save(os);
  ff2.save(os);
...
void TransformerMHDecoderLayer::load(std::istream &is) {
  attention.load(is);
  norm1.load(is);
  ff1.load(is);
  ff2.load(is);
...
MultiHeadAttentionLayer::MultiHeadAttentionLayer(int embed_dim, int seq_len,
                                                 int num_heads)
    : embed_dim(embed_dim), seq_len(seq_len), num_heads(num_heads),
      head_dim(embed_dim / num_heads), W_q(embed_dim, embed_dim),
      W_k(embed_dim, embed_dim), W_v(embed_dim, embed_dim), b_q(1, embed_dim),
...
const Tensor &MultiHeadAttentionLayer::forward(const Tensor &input) {
  ...
  ThreadPool::getInstance().parallel_for(
      0, batch_size * num_heads, [this](int task_idx) {
        int b = task_idx / num_heads;
        int h = task_idx % num_heads;
        int head_offset = h * head_dim;
...
const Tensor &MultiHeadAttentionLayer::backward(const Tensor &input,
                                                const Tensor &grad_output) {
  int batch_size = input.rows / seq_len;
  ensure_grad_input_dims(input.rows, input.cols);
...
void MultiHeadAttentionLayer::save(std::ostream &os) const {
  W_q.save(os);
  W_k.save(os);
  W_v.save(os);
  W_o.save(os);
...
void MultiHeadAttentionLayer::load(std::istream &is) {
  W_q.load(is);
  W_k.load(is);
  W_v.load(is);
  W_o.load(is);
...
TransformerMHEncoderLayer::TransformerMHEncoderLayer(int embed_dim, int seq_len,
                                                     int ff_dim, int num_heads,
                                                     scalar dropout_rate)
    : embed_dim(embed_dim), seq_len(seq_len),
      ff_dim(ff_dim == -1 ? 4 * embed_dim : ff_dim), num_heads(num_heads),
      attention(embed_dim, seq_len, num_heads), norm1(embed_dim),
      drop1(dropout_rate), ff1(embed_dim, this->ff_dim),
      relu(&relu_fn_mhenc_inst), ff2(this->ff_dim, embed_dim),
      drop2(dropout_rate), norm2(embed_dim) {}
...
const Tensor &TransformerMHEncoderLayer::backward(const Tensor &input,
                                                  const Tensor &grad_output) {
  ensure_grad_input_dims(input.rows, input.cols);
...
void TransformerMHEncoderLayer::save(std::ostream &os) const {
  attention.save(os);
  norm1.save(os);
  ff1.save(os);
  ff2.save(os);
...
void TransformerMHEncoderLayer::load(std::istream &is) {
  attention.load(is);
  norm1.load(is);
  ff1.load(is);
  ff2.load(is);
...
const Tensor &GlobalAveragePooling1DLayer::backward(const Tensor &input,
                                                    const Tensor &grad_output) {
  int batch_size = input.rows / seq_len;
