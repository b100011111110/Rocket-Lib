...
public:
  ...
  double forward(const Tensor &y_pred, const Tensor &y_true) override {
    PYBIND11_OVERRIDE_PURE(double, Loss, forward, y_pred, y_true);
  }
...
PYBIND11_MODULE(rocket, m) {
  ...
  py::class_<Tensor>(m, "Tensor")
      ...
      .def("set_val",
           [](Tensor &t, int i, int j, double val) {
  ...
  py::class_<RegularizationLayer, Layer>(m, "RegularizationLayer")
      .def(py::init<double, int>(), py::arg("lambda"), py::arg("type") = 2);
  ...
  py::class_<SGD, Optimizer>(m, "SGD").def(py::init<double>(),
                                           py::arg("lr") = 0.01);

  py::class_<Adam, Optimizer>(m, "Adam").def(
      py::init<double, double, double, double>(), py::arg("lr") = 0.001,
